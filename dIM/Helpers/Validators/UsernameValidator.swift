...
class UsernameValidator: ObservableObject {
    ...
    func validate(username: String) -> State {
        ...
        guard username.count >= 4 else { return .error(message: "用户名太短了！") }
        guard username.count <= 16 else { return .error(message: "真的需要这么长的名字吗？") }
        guard !username.contains(" ") else { return .error(message: "用户名不得包含空格。")}
        guard !username.contains("#") else { return .error(message: "用户名不得包含 #")}
        guard !username.contains("/") else { return .error(message: "用户名不得包含 /")}
